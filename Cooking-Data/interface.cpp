...
void Interface::updateObject(int index, const b2Body* source)
{
    ...
    sprites[sprites.size() - index - 1].second.setPosition(QPoint(source->GetPosition().x * SCALE - boxSize.width()/2,
                                             source->GetPosition().y * SCALE - boxSize.height()/2));
}
...
void Interface::mousePressEvent(QMouseEvent* event){
    ...
    for (int i = 0; i < sprites.size(); i++){
        ...
        int x2 = item.second.getPosition().x() + item.second.getDimensions().width();
        int y2 = item.second.getPosition().y() + item.second.getDimensions().height();
        ...
        if(x1 <= mouseLoc.x() && x2 >= mouseLoc.x()
           && y1 <= mouseLoc.y() && y2 >= mouseLoc.y()){
            ...
            qDebug() << "Emit";
            emit draggableClicked(sprites.size() - i - 1, mouseLoc.x() / SCALE, mouseLoc.y() / SCALE);
...
void Interface::mouseMoveEvent(QMouseEvent* event){
    if(mouseIsDown)
        emit draggableClicked(selectedObjectIndex, event->pos().x() / SCALE, event->pos().y() / SCALE);
}
